#![no_std]
use gstd::{msg, prelude::*, ActorId, prog::ProgramGenerator, CodeId, exec};
...
#[gstd::async_main]
async fn main() {
    let action: TamagotchiFactoryAction = msg::load()
    ...
    match action {
        ...
        TamagotchiFactoryAction::ApproveUser { tamagotchi_id, user } => {
        ...
        TamagotchiFactoryAction::SetFTTokenContract { tamagotchi_id, ft_token_contract } => {
            factory.set_ft_token_contract_to_tamagotchi(tamagotchi_id, ft_token_contract).await;
        },
        TamagotchiFactoryAction::ApproveTokens { tamagotchi_id, user, amount } => {
            factory.approve_tokens_from(tamagotchi_id, user, amount).await;
        },
        TamagotchiFactoryAction::BuyAttributeToTamagotchi { tamagotchi_id, store_id, attribute_id } => {
            factory.buy_attribute_to_tamagotchi(tamagotchi_id, store_id, attribute_id).await
        },
        TamagotchiFactoryAction::CheckTamagotchiState(tamagotchi_id) => {
            factory.check_tamagotchi_state(tamagotchi_id).await;
        },
        TamagotchiFactoryAction::ReserveGasToTamagotchi { tamagotchi_id, reservation_amount, duration } => {
            factory.reserve_gas_to_tamagotchi(tamagotchi_id, reservation_amount, duration).await;
        }
    }
}
